...
router.get('/csansan/users-csansan', function *(next) {
  this.body = db.users;
});

router.get('/csansan/users-csansan/:userId', function *(next) {
...
router.get('/csansan/posts-csansan', function *() {
  this.body = db.posts;
});

router.get('/csansan/posts-csansan/in-thread/:threadId', function *() {
  const id = parseInt(this.params.threadId);
  this.body = db.posts.filter((post) => post.thread == id);
});

router.get('/csansan/posts-csansan/by-user/:userId', function *() {
  const id = parseInt(this.params.userId);
